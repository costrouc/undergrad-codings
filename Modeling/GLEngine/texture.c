...
{
  GLuint *bufTexture = malloc(sizeof(GLuint) * 1);
  glGenTextures(1,bufTexture);

  //Copy texture to the buffer
  glBindTexture(GL_TEXTURE_2D,bufTexture[0]);
  glTexImage2D(GL_TEXTURE_2D,0,3,t->_width,t->_height,0,GL_RGB,GL_UNSIGNED_BYTE,t->_data);
  ...
  //look at code these should be placed elsewhere
