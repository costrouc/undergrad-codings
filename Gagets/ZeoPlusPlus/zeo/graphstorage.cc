...
const DELTA_POS DELTA_POS::absoluteValue() const   { return DELTA_POS(abs(x), abs(y), abs(z));}
const bool DELTA_POS::isZero() const               { return (x == 0) && (y == 0) && (z == 0);}
const double DELTA_POS::magnitude() const          { return sqrt(x*x + y*y + z*z);}
void DELTA_POS::print(ostream &out) const          { out << x << " " << y << " " << z << "\n";}
 ...
 */
void DIJKSTRA_NETWORK::buildDijkstraNetwork(VORONOI_NETWORK *vornet, DIJKSTRA_NETWORK *dnet){
