...
void* performVoronoiDecomp(bool radial, ATOM_NETWORK *cell, VORONOI_NETWORK *vornet, vector<VOR_CELL> &cells, bool saveVorCells,
  ...
  if(radial) {
    ...
    storeVoronoiNetwork(*rad_con, cell, vornet, bx, by, bz, bvcells, atomShifts, saveVorCells, cells);
    vornet->v_a = cell->v_a;
    vornet->v_b = cell->v_b;
    vornet->v_c = cell->v_c;
    return rad_con;
    ...
    // Store the Voronoi Network for later use
    storeVoronoiNetwork(*no_rad_con, cell, vornet, bx, by, bz, bvcells, atomShifts, saveVorCells, cells);
    vornet->v_a = cell->v_a;
    vornet->v_b = cell->v_b;
    vornet->v_c = cell->v_c;
    return no_rad_con;
...
void storeVoronoiNetwork(c_option &con, ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet, double bx, double by, double bz,
...
//    }
    ...
    double error_percent_tolerance = 0.1;
