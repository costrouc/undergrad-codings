...
//    exit(0);
  ...
  if(!error) {
    ...
    if(!error) {
      ...
      bool radial = true, outputZvis = false, buildBasicCells = false, useMass = true, highAccuracy = false;
      ...
      */
	    ...
	    for(unsigned int i=0; i < channels.size(); i++){
          ...
          // This include di and positions of nodes
            ...
            calcAV(&atmnet, &orgAtomnet, highAccuracy, probe_radius, probe_radius, numSamples, true, output, (char *)filename.c_str(), 0, 0, 0, 0, -1,-1);
...
//            calculateAverageGrid(&atmnet, filename_InputData, filename_Gaussian_cube, angstrom_to_bohr, useMass);
          ...
          else if( (command[0].compare("-vol") == 0) || (command[0].compare("-zvol") == 0) || (command[0].compare("-vvol") == 0) || (command[0].compare("-lvol") == 0)) {
  ...
  //              exit(EXIT_FAILURE);
            ...
            calcAV(&atmnet, &orgAtomnet, highAccuracy, chan_radius, probe_radius, numSamples, true, output, (char *)filename.data(), visualize, VisITflag, LiverpoolFlag, blockingMode, low_dist_range, high_dist_range);
            output.close();
          }
          else if( (command[0].compare("-block") == 0) ) {
            int estAVnumSamples = 50000;
            string suffix;
            suffix = ".block";
            double probe_radius  = strtod(command[1].data(), NULL);
  ...
  //            exit(EXIT_FAILURE);
            ...
            fstream output; 
            output.open(filename.data(), fstream::out);
            calcAV(&atmnet, &orgAtomnet, highAccuracy, probe_radius, probe_radius, numSamples, true, output, (char *)filename.data(), 0, 0, 0, 1, low_dist_range, high_dist_range);
            output.close();
          }
