...
bool checkInputFile(char * filename){
  ...
  string fileTypes [] = {".cuc", ".arc", ".cssr", ".v1",".cif"};
  int numTypes = 5;
...
//printf("DEBUG: exited loop successfully, having read data line \"%s\"\n", line.c_str());
            ...
            while(token.size()>0){
              ...
              if (token.size()!=column_labels.size() && column_labels[0].compare(list[8]) != 0 && column_labels[0].compare(list[9]) != 0){
                goto exception; //unexpected input
              }
  ...
  //printf("SYM DEBUG: symmetry section found!\n");
	              if (token.size()!=3){
	                cerr << "Error: Expected 3 strings for _symmetry_equiv_pos_as_xyz" << endl;
  //	              abort();
                  ciffile.close();
                  return false;
	              }
...
bool readCSSRFile(char *filename, ATOM_NETWORK *cell, bool radial){
  ...
  if(input.is_open()==true){
    ...
    string numStr;
    bool longCSSR=false;  // this flag with enable switching to a different read routine
...
bool writeToCIF(char *filename, ATOM_NETWORK *cell){
  ...
  if(!output.is_open()){
    ...
    cout   << "Writing atom network information to " << filename << "\n";
    if (cell->name.compare("NULL")==false){
      output << "data_" << cell->name << endl;
    }
    output << "#******************************************" << endl;
    output << "#" << endl;
    ...
    output << "#" << endl;
    output << "#*******************************************" << "\n\n";
    output << "_cell_length_a\t\t" << cell->a << "(0)" << endl;
    output << "_cell_length_b\t\t" << cell->b << "(0)" << endl;
    output << "_cell_length_c\t\t" << cell->c << "(0)" << endl;
    output << "_cell_angle_alpha\t\t" << cell->alpha << "(0)" << endl;
    output << "_cell_angle_beta\t\t" << cell->beta << "(0)" << endl;
    output << "_cell_angle_gamma\t\t" << cell->gamma << "(0)\n\n";
...
bool writeToVTK(char *filename, ATOM_NETWORK *cell){
  ...
  if(!output.is_open()){
    ...
    output << "# vtk DataFile Version 2.0\n%s - vtk format representation of unit cell boundary\nASCII\nDATASET POLYDATA\nPOINTS 8 double\n";
