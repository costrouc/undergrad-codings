...
/*
  output << "Pore size distribution Histogram - Bin Size = "<<binSize<<" Number of Bins: "<<maxBins<<" From: 0 To: "<<numBin<<"\nAccessible samples: "<<count<<" Fraction of sample points in node spheres: "<<nodefrac<<"\nFraction of sample points outside node spheres: "<<outfrac<<"\n\nBin\tCount\tCum. Dist\tDer. Dist\n";
  for (int i=0; i<maxBins; i++){
    output<<binSize*1.0*i<<"\t"<<setw(6)<<bins[i]<<"\t"<<setw(10)<<cumBins[i]<<"\t"<<setw(12)<<derivBins[i]<<"\n";
  }
*/
